...
class XdrReader {
    constructor(source) {
        ...
        if (!Buffer.isBuffer(_source)) {
            if (_source instanceof Array) {
                ...
                throw new errors_1.XdrReaderError("source not specified");
            }
        }
    ...
    advance(size) {
        ...
        if (this._length < this._index)
            throw new errors_1.XdrReaderError("attempt to read outside the boundary of the buffer");
        ...
        if (padding > 0) {
            ...
            for (let i = 0; i < padding; i++)
                if (this._buffer[this._index + i] !== 0)
                    ...
                    throw new errors_1.XdrReaderError("invalid padding");
            this._index += padding;
        }
        return from;
    }
    ...
    ensureInputConsumed() {
        if (this._index !== this._length)
            throw new errors_1.XdrReaderError(`invalid XDR contract typecast - source buffer not entirely consumed`);
    }
}
